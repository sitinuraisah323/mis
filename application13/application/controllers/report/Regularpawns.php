...
class Regularpawns extends Authenticated
{
	...
	{		
		...
		$objPHPExcel->getActiveSheet()->setCellValue('O1', 'Description');
		 ...
		 $this->regulars->db
			->select('units.name as unit, customers.name as customer_name,customers.nik as nik, units.code')
			->join('customers','units_regularpawns.id_customer = customers.id')
			->join('units','units.id = units_regularpawns.id_unit');
			// ->join('units_repayments', 'units_regularpawns.no_sbk=units_repayments.no_sbk')
			// ->where('no_sbk', 'units_repayments.no_sbk')
			// ->where('amount', 'units_repayments.money_loan');
			// ->where('permit', 'units_repayments.permit');
		...
		if($post = $this->input->post()){
			...
			$status=["N","L"];
			$this->regulars->db
				// ->where('units_regularpawns.date_repayment', NULL)
				->where('units_regularpawns.date_sbk <=', $post['date-end'])
				->where_in('units_regularpawns.status_transaction ', $status);
			if($post['id_unit']){
				$this->regulars->db
			...
			if($sortBy = $this->input->post('sort_by')){
				$this->regulars->db->order_by('units_regularpawns.'.$sortBy, $this->input->post('sort_method'));
			}
			if($type = $this->input->post('type')){
				$this->regulars->db->where('units_regularpawns.type_bmh', $type === 'OPSI' ? 'RB' : 'RC');
			}
		}
		if($no_sbk = $this->input->post('no_sbk')){
			$this->regulars->db->where('units_regularpawns.no_sbk',  $no_sbk);
		}

		// if($unit = $this->input->post('dateEnd')){
		// 	$null = $this->regulars->db->where('units_regularpawns.date_repayment', NULL);
		// } 
		$data = $this->regulars->all();
		// var_dump($data); exit;
		$no=2;
		...
		foreach ($data as $row) 
		{
			$statusrpt = $this->input->post('status');;
			$date = $this->input->post('date-end');
						if($row->date_repayment>$date ){
                            $status="Aktif";
                        }else if($row->date_repayment<=$date){
                            $status="Lunas";
                        }
		...
		//Redirect output to a client’s WBE browser (Excel5)
		$filename ="Detail_Outstanding" .date('d/m/Y', strtotime($date));
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment;filename="'.$filename.'.xls"');
		header('Cache-Control: max-age=0');
		$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
