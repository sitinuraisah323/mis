...
class Outstanding extends Authenticated
{
	...
	public function calculate_repayment($date)
	{
		...
		foreach($units as $index => $unit){
			...
			')
				...
				->where('urm.id_unit', $unit->id)
				->get()->row()->noa;
			// var_dump($disburseNoaReg); exit;
			
			// $disburseUpMor = (int) $this->mortages->db->select('sum(m.amount_loan) as up,
			// count(id) as noa
			// ')
			// 	->from('units_mortages m')
			// 	->where('year(rm.date_rep)', $explode[0])
			// 	->where('status_transaction', 'L')
			// 	// ->where('urm.date_repayment <=', $date)
			// 	->where('urm.id_unit', $unit->id)
			// 	->get()->row()->up;
			// // var_dump($disburseUp); exit;

			// $disburseNoaMor = (int) $this->mortages->db->select('sum(m.amount_loan) as up,
			// count(id) as noa
			// ')
			// 	->from('units_mortages m')
			// 	->where('year(rm.date_rep)', $explode[0])
	...
	{		
		...
		// 	->get()->result();
		// foreach($units as $index => $unit){
		// $disburse = $this->repayments->db->select('sum(urm.money_loan) as up,
		// 	count(urm.id) as noa
		// 	')
		// 		->from('units_repayments urm')
		// 		// ->where('month(urm.date_repayment)', $explode)
		// 		// // ->where('urm.date_repayment <=', $date)
		// 		// ->where('urm.id_unit', $unit->id)
		// 		// ->get()->row()->up;
		// 		->join('units','units.id =urm.id_unit ')
		// 		->join('areas','areas.id = units.id_area')
		// 		->where('units.id', $unit->id)
		// 		->where('year(urm.date_repayment)', $explode[2])
		// 		// ->where('date', date('Y-m-d', strtotime($curr)) )
		// 		->get()->result();
		// }
// 		$disburse = $this->units_repayments->db
// 			->select('count (units_repayments.id) as noa, sum(units_repayments.amount_loan) as reg')
// 			->from('units_repayments')
// 			->join('units','units.id =units_repayments.id_unit ')
// 			->join('areas','areas.id = units.id_area')
// 			->where('year', date('Y-m-d', strtotime($curr)) )
// 			->get()->result();
// var_dump($disburse); exit;
		//$os = $this->data();
