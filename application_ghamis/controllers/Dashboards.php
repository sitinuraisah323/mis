...
class Dashboards extends Authenticated
{
	...
	public function update_date(){
		$this->db->update('units_regularpawns', 'rp.date_repayments=rm.date_repayments')
		->join('units_repayments rm', 'rp.no_sbk= rm.no_sbk')
		->join('units_regularpawns rp', 'id=rp.id')
		->update('units_regularpawns rp', 'rp.date_repayment=rm.date_repayment')
		->where('rp.permit', 'rm.permit')
		->where('rp.date_sbk', 'rm.date_sbk')
		->where('rp.id_unit', 'rm.id_unit');
	}
	...
	public function januari()
	{
        ...
        $data['areas'] = $this->areas->all();
		$data['month'] = 1;
		$this->load->view("dashboard/januari/index", $data);
