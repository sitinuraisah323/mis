...
function initCariForm() {
    ...
    $('#btncari').on('click', function() {
        ...
        $.ajax({
            type: 'GET',
            url: "<?php echo base_url("api/transactions/regularpawns/detail_os"); ?>",
            dataType: "json",
            data: {
                no_sbk,
                area: area,
            ...
            success: function(response, status) {
                ...
                if (response.status == true) {
                    ...
                    $.each(response.data, function(index, data) {
                        if (data.date_repayment > dateEnd) {
                            status = "Aktif";
                        } else if (data.date_repayment <= dateEnd) {
                            status = "Lunas";
                        } else {
                            status = "Aktif";
                        }
                        if (statusrpt == "1") {
                            if (status == "Lunas") {
                                ...
                                template += "<td class='text-center'>" + data.unit +
                                "</td>";
                                template += "<td class='text-center'>" + data.nic + "</td>";
                                template += "<td class='text-center'>" + data.no_sbk +
                                    "</td>";
                                template += "<td class='text-center'>" + moment(data
                                    .date_sbk).format('DD-MM-YYYY') + "</td>";
                                template += "<td class='text-center'>" + moment(data
                                    .deadline).format('DD-MM-YYYY') + "</td>";
                                if (data.date_repayment) {
                                    var DateRepayment = moment(data.date_repayment).format(
                                        'DD-MM-YYYY');
                                } else {
                                    var DateRepayment = "-";
                                }
                                ...
                                //     DateRepayment="-";
                                // }
                                template += "<td class='text-center'>" + DateRepayment +
                                    "</td>";
                                template += "<td>" + data.customer_name + "</td>";
                                template += "<td class='text-center'>" + data
                                    .capital_lease + "</td>";
                                ...
                                if (data.description_1 != null) {
                                    template += "- " + data.description_1;
                                }
                                if (data.description_2 != null) {
                                    template += "<br>- " + data.description_2;
                                }
                                if (data.description_3 != null) {
                                    template += "<br>- " + data.description_3;
                                }
                                if (data.description_4 != null) {
                                    template += "<br>- " + data.description_4;
                                }
                                template += "</td>";
                                template +=
                                    `<td class='text-center'>${data.type_bmh === 'RC' ? 'REGULER' : 'OPSI'}</td>`;
                                template += '</tr>';
                                no++;
                            ...
                            template += "<td class='text-center'>" + moment(data.date_sbk)
                                .format('DD-MM-YYYY') + "</td>";
                            template += "<td class='text-center'>" + moment(data.deadline)
                                .format('DD-MM-YYYY') + "</td>";
                            if (data.date_repayment) {
                                var DateRepayment = moment(data.date_repayment).format(
                                    'DD-MM-YYYY');
                            ...
                            "</td>";
                            template += "<td>" + data.customer_name + "</td>";
                            template += "<td class='text-center'>" + data.capital_lease +
                                "</td>";
                            template += "<td class='text-right'>" + convertToRupiah(data
                                .estimation) + "</td>";
                            template += "<td class='text-right'>" + convertToRupiah(data
...
//     var a = '';
//     var angka1 = angka.toString().split('').reverse().join('');
//     for(var i = 0; i< angka1.length; i++) if(i%3==0) rupiah += angka1.substr(i,3)+'.';
//     return rupiah.split('',rupiah.length-1).reverse().join('');
// }

//     $('[name="area"]').on('change', function(){
//         var cabang = $('[name="cabang"]').val();
//         var unit = $('[name="cabang"]');
//         var url = $('[name="url"]')+url+cabang;
...
//     $.ajax({
//         type : 'GET',
//         url : "<?php echo base_url("api/datamaster/transactions"); ?>",
//         dataType : "json",
//         data:{unit:unit},
//         success : function(response, status){
//             KTApp.unblockPage();
...
//         },
//         complete: function(){
//             KTApp.unblock('#form_bukuka .kt-portlet__body', {});
//         }
//     });
// }
