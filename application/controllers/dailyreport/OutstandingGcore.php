...
class Outstanding extends Authenticated
{
	...
	public function calculate_smartphone($date){
		...
		foreach($units as $index => $unit){
			...
			count(id) as noa
			')
				->from('units_regulapawns reg')
				->where('reg.date_sbk >=', $dateStart)
				->where('reg.date_sbk <=', $date) 
				->where('reg.id_unit', $unit->id)
				->get()->row()->up;
			
			// var_dump($akumulasiUP); exit;
			$todayUp = (int) $this->regular->db->select('sum(reg.amount) as up,
				count(id) as noa
				')
				->from('units_regularpawns reg')
				->where('get.date_repayment', $date)
				->where('get.id_unit', $unit->id)
				->get()->row()->up;

			
		
			$akumulasiNoa = (int) $this->regular->db->select('sum(reg.amount) as up,
			count(id) as noa
			')
				->from('units_regularpawns reg')
				->where('reg.date_sbk >=', $dateStart)
			...
			$todayNoa = (int) $this->regular->db->select('sum(reg.amount) as up,
				count(id) as noa
				')
				->from('units_regularpawns reg')
				->where('reg.date_sbk', $date)
		...
		if(count($insert)){
			$this->regular->db->insert_batch('smartphone', $insert);
		}
		if(count($update)){
			$this->regular->db->update_batch('smartphone', $update, 'id');
		}
		return (object) $data;
	}
	...
	{		
		...
		// $smartphone = $this->smartphone->db
		// ->select('areas.area, units.name as unit, date, noa_ak, up_ak, noa_to, up_to')
		// ->from('smartphone')
		// ->join('units', 'units.id = smartphone.id_unit')
		// ->join('areas', 'areas.id = units.id_area')
		// ->where('date', date('Y-m-d', strtotime($curr)))
		...
		// if(count($smartphone)){
		// 	$this->calculate_smartphone($curr);
		// 	$smartphone = $this->smartphone->db
		// 	->select('units.name as unit, areas.area, date, noa_ak, up_ak, noa_to, up_to')
		// 	->from('smartphone')
		// 	->join('units', 'units.id = smartphone.id_unit')
		// 	->join('areas', 'areas.id = units.id_area')
		// 	->where('date', date('Y-m-d', strtotime($curr)))
		...
		// $this->load->library('areasSiscol');
		// $area = $this->areasSiscol->areas();
		// var_dump($area); exit;

	//Outstanding gcore
		// $this->load->library('gcore');
		// $areas = $this->gcore->areas()->data;
	//end outstanding gcore

		// foreach($areas as $data){
		// 	// var_dump($data->id);
		// 	// exit;
		// 	// $areas = ['6164d6fc772ace3496debe5c',]
			// $data =  $this->gcore->transaction($curr, $area->id, $branch, $unit, 0); 
		// 	$osSiscol = $data;
		// 	var_dump($osSiscol);
		// 	echo "========================================================================================";
		...
		// $area = $this->gcore->areas(); 

		// var_dump($osSiscol); 
		// exit;

		//Outstanding
		// $panak = "6164d6fc772ace3496debe5c";
	...
	//Os ghanet aja	
		...
		$pdf->AddPage('L', 'A3');
		$view = $this->load->view('dailyreport/outstanding/generate.php',['outstanding'=>$grouped,'datetrans'=> $curr, 'osSiscol' => $osSiscol, 'panakukang' => $panakukang],true);
		$pdf->writeHTML($view);
	//end OS ghanet aja
		...
		// $smartphone = $this->regular->db
		// ->select('units.name, areas.area, units_regularpawns.*')
		// ->from('units_regularpawns')
		// ->join('units', 'units.id = units_regularpawns.id_unit')
		// ->join('areas', 'areas.id = units.id_area')
		// ->like('description_1', 'HP')
		// ->where('date_sbk', date('Y-m-d', strtotime($curr)))->get()->result();

		// $pdf->AddPage('L','A3');
		// $view = $this->load->view('dailyreport/outstanding/smartphone.php', ['smartphone'=>$smartphone, 'datetrans'=>$curr], true);
		// $pdf = writeHTML($view);
