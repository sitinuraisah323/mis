	...
	{		
		...
		if(count($repayments) <= 0){
			...
			today_over_loan, disburse_noa, disburse_up')
				...
				->join('units','units.id = units_repayments_summaries.id_unit ')
				->join('areas','areas.id = units.id_area')
				->where('date_sbk', date('Y-m-d', strtotime($curr)) )
				->get()->result();
		}

		$smartphone = $this->regular->db
		->select('units.name as unit, areas.area, date_sbk, no_sbk, amount')
		...
		$units = $this->units->db->select('id, name')->from('units')
			->get()->result();
		$dateStart = date('Y-m-d', strtotime(date('Y-m-01', strtotime($curr))));
		$date = date('Y-m-d', strtotime($curr));
		$insert = [];
		$update = [];
		$data = [];
		foreach($units as $index => $unit){
			...
			->select('units.name as unit, areas.area, date_sbk, no_sbk, amount')
			->from('units_regularpawns')
			->join('units', 'units.id = units_regularpwns.id_unit')
			->join('areas', 'areas.id = units.id_area')
			->where('date_sbk', date('Y-m-d', strtotime($curr)))
			->get()->result();
		}
		...
		$pdf->AddPage('L', 'A3');
		$view = $this->load->view('dailyreport/outstanding/smartphone.php',['smartphone'=>$smartphone, 'akumulasiUp'=>$akumulasiUp, 'akumulasiNoa'=>$akumulasiUp, 'todayUp'=>$todayUp, 'datetrans'=> $curr], true);
		$pdf->writeHTML($view);
