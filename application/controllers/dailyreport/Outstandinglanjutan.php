...
class Outstandinglanjutan extends Authenticated
{
	...
	public function gcore_grouped($os)
	{
		$result = [];
		foreach($os as $index =>  $data){
			$result[$data->area_id][$index] = $data;
		}
		return $result;
	}
...
// 		//Pendapatan Gcore
// 		$pdf->AddPage('L', 'A4');
// 		$view = $this->load->view('dailyreport/outstanding/gcore_pendapatan.php', ['pendapatan' => $this->gcore_pendapatan(),
// 		'datetrans' => $curr], true);
// 		$pdf->writeHTML($view);
...
//		pengeluaran Gcore
	...
	//  $data = $this->swamandiri_gcda();
	// //  var_dump($data); exit;
	// foreach($data as $d => $min){
	// 	foreach($min as $n){
	// 	echo $n->office_name;
	// 	exit;
	// 	}
...
// SWAMANDIRI RATE
	...
	// $pdf->AddPage('L','A4');
	// 	// $group = $this->gcore_grouped($this->gcore_rate());
	// 	var_dump($this->gcore_oneobligor()); 

	// 	$view = $this->load->view('dailyreport/outstanding/gcore_oneobligor.php',['oneobligor'	=> $this->gcore_oneobligor()],true);
	// 	$pdf->writeHTML($view);
...
public function reportoutstanding()
	...
	public function get_lainnya($unit, $region)
	{
		...
		->where('trans.created_at <=', $date_end)
	...
	{
		$yesterday = date('Y-m-d', strtotime('+1 days', strtotime($this->datetrans())));
		$date_end = date('Y-m-d', strtotime($this->datetrans()));
		$date = date('Y-m-01');
	...
	public function gcore_oneobligor()
	{
		$rate = 1.90;
		$selisih = 0.06;
		
		$data = $this->db2
		->select("office_code, pawn_transactions.area_id,office_name, cif_number, sge, customers.name, contract_date, due_date, auction_date, maximum_loan,loan_amount, admin_fee, interest_rate, interest_rate - '$rate' as selisih"
			)
		->from('pawn_transactions')
		->inner_join('customers', 'customers.id=pawn_transactions.customer_id')
		->where('area_id', '60c6bfcce64d1e242863024a')
		->where('contract_date >=' , '2022-06-01' )
		->where('interest_rate >', $rate)
		->where("interest_rate - '$rate' >=", 0.06)
