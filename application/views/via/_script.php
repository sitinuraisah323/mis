...
function initDTEvents(){
    $(".btn_delete").on("click",function(){
        ...
        swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax({
                    type : 'GET',
                    url : "<?php echo base_url("api/datamaster/via/delete"); ?>",
                    data : {id:targetId},
                    dataType : "json",
    ...
    $(".btn_edit").on("click",function(){
        ...
        $.ajax({
            type : 'GET',
            url : "<?php echo base_url("api/datamaster/via/get_byid"); ?>",
            data : {id:targetId},
            dataType : "json",
...
function initDataTable(){
    var option = {
        data: {
            ...
            source: {
              read: {
                url: '<?php echo base_url("api/datamaster/via"); ?>',
                map: function(raw) {
                  // sample data mapping
                  var dataSet = raw;
...
function initCreateForm(){
    ...
    $('#add_area').select2({
        placeholder: "Please select a Jenis Payments",
        width: '100%'
    }); 
    ...
    $("#btn_add_submit").on("click",function(){
      ...
      if(isValid){
        ...
        $.ajax({
            type : 'POST',
            url : "<?php echo base_url("api/datamaster/via/insert"); ?>",
...
function initEditForm(){
    ...
    $("#btn_edit_submit").on("click",function(){
      ...
      if(isValid){
        ...
        $.ajax({
            type : 'POST',
            url : "<?php echo base_url("api/datamaster/via/update"); ?>",
    ...
    var populateForm = function(groupObject){
        ...
        $("#edit_area_name").val(groupObject.id_payments);
        $('#edit_area_name').select2({
        placeholder: "Please select a Jenis Payments",
        width: '100%'
        });
        $("#edit_area_via").val(groupObject.via);
