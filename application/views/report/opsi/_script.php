...
function initCariForm(){
    ...
    $('#btncari').on('click',function(){
		...
		$.ajax({
            ...
            dateStart:dateStart,dateEnd:dateEnd,permit:permit, sort_by, sort_method, type:"OPSI"},
			success : function(response,status){
				...
				if(response.status == true){
					...
					$.each(response.data, function (index, data) {
						...
						template += `<td class='text-center'>${data.periode < 21 && data.periode != 0 ? 'Mingguan' : 'Bulanan'}</td>`;
