...
function initCariForm(){
    ...
    $('#permit').select2({ placeholder: "Select a permit", width: '100%' });
    $('#type').select2({ placeholder: "Select a type", width: '100%' });
    //events
    $('#btncari').on('click',function(){
		...
		$.ajax({
			type : 'GET',
			url : "<?php echo base_url("api/transactions/regularpawns/transactionmiss"); ?>",
			dataType : "json",
			data:{id_area:area,cabang:cabang,id_unit:unit,statusrpt:statusrpt, month, year, permit, type},
			success : function(response,status){
				...
				if(response.status == 200){
                    ...
                    var status="";
                    if(type == 'CASH'){
                        $('#type-transaction').addClass('d-none');
                        ...
                        $('#type-transaction').removeClass('d-none');
                        ...
                        $.each(response.data, function (index, data) {
                            ...
                            template += "<td class='text-right'>"+status+"</td>";
                            template += '</tr>';
