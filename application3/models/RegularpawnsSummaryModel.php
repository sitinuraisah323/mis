...
class RegularpawnsSummaryModel extends Master
{
	...
	public function getTotalGramation($idUnit)
	{
		...
		$getUp = $this->db->select('sum(amount) as up,sum(estimation) as estimation, count(*) as noa')->from('units_regularpawns_verified')
							  ...
							  ->where('units_regularpawns.id_unit', $idUnit)->get()->row();		
