...
function initCariForm(){
    ...
    $('#btncari').on('click',function(){
		...
		$.ajax({
			type : 'GET',
			url : "<?php echo base_url("api/report/konversi/outstanding"); ?>",
			dataType : "json",
			...
			success : function(response,status){
				...
				if(response.status == true){
					...
					$.each(response.data, function (index, data) {
						...
						template += "<td class='text-right'>"+convertToRupiah(data.bapkas.os_bap)+"</td>";                        
						template += "<td class='text-right'>"+convertToRupiah(data.bapkas.os_real)+"</td>";
						template += "<td class='text-right'>"+convertToRupiah(data.bapkas.os)+"</td>";
                        selisih = data.bapkas.os - data.bapkas.os_bap;
                        ghanet_x_real = data.bapkas.os - data.bapkas.os_real;
                        os_x_real = data.bapkas.os_bap - data.bapkas.os_real;
						template += "<td class='text-right'>"+convertToRupiah(selisih)+"</td>";
						template += "<td class='text-right'>"+convertToRupiah(ghanet_x_real)+"</td>";
						template += "<td class='text-right'>"+convertToRupiah(os_x_real)+"</td>";
						template += '</tr>';
                        totbap += data.bapkas.os_bap;
                        totos += data.bapkas.os;
                        totreal += data.bapkas.os_real;
                        totselisih += selisih;
                        totselisihreal += ghanet_x_real;
                        totselisihos += os_x_real;
						no++;
					});                   
					    template += "<tr class='rowappend'>";
						template += "<td class='text-center' colspan='3'>Total</td>";
						template += "<td class='text-right'>"+convertToRupiah(totbap)+"</td>";                        
						template += "<td class='text-right'>"+convertToRupiah(totreal)+"</td>";
						template += "<td class='text-right'>"+convertToRupiah(totos)+"</td>";
						template += "<td class='text-right'>"+convertToRupiah(totselisih)+"</td>";
						template += "<td class='text-right'>"+convertToRupiah(totselisihreal)+"</td>";
						template += "<td class='text-right'>"+convertToRupiah(totselisihos)+"</td>";
						template += '</tr>';
